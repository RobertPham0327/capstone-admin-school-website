...
mock.onPost('/api/scrumboard/add/board').reply((request: AxiosRequestConfig) => {
...
mock.onPut('/api/scrumboard/edit/board').reply((request: AxiosRequestConfig) => {
  const { board } = JSON.parse(request.data);
...
mock.onPut('/api/scrumboard/edit/list').reply((request: AxiosRequestConfig) => {
  ...
  const updatedBoardList = boardData.map(data => {
    if (data.id === boardId) {
      data.list = data.list.map((item: any) => (item.id === list.id ? list : item));
      return data;
    }
    return data;
  });
...
mock.onPut('/api/cards/update/category').reply((request: AxiosRequestConfig) => {
  ...
  const updatedBoardList = boardData.map((data: BoardObjType) => {
    if (data.id === boardId) {
      const sourceLane = data.list.find((item: CardListObjType) => item.id === sourceLaneId);
      const card = sourceLane?.cards.find((item: CardObjType) => item.id === cardId);
      if (sourceLane) {
        sourceLane.cards = sourceLane.cards ? sourceLane.cards.filter(item => item.id !== cardId) : [];
      }
      const targetLane = data.list.find((item: CardListObjType) => item.id === categoryId);
      if (targetLane?.cards) {
...
mock.onPost('/api/scrumboard/add/card').reply((request: AxiosRequestConfig) => {
  ...
  const selectedBoard: BoardObjType = boardData.find(data => data.id === board.id)!;
  const selectedList: CardListObjType = selectedBoard.list.find(data => data.id === list.id)!;
...
mock.onPut('/api/scrumboard/edit/card').reply((request: AxiosRequestConfig) => {
  ...
  const selectedBoard: BoardObjType = boardData.find(data => data.id === board.id)!;
  const selectedList: CardListObjType = selectedBoard.list.find(data => data.id === list.id)!;
  selectedList.cards = selectedList.cards.map((data: CardObjType) => (data.id === card.id ? card : data));
  selectedBoard.list = selectedBoard.list.map(data => (data.id === selectedList.id ? selectedList : data));
  boardData = boardData.map(data => (data.id === selectedBoard.id ? selectedBoard : data));
  return [200, selectedBoard];
});

mock.onPost('/api/scrumboard/delete/card').reply((request: AxiosRequestConfig) => {
  const { boardId, listId, cardId } = JSON.parse(request.data);
  const selectedBoard: BoardObjType = boardData.find(data => data.id === boardId)!;
  const selectedList: CardListObjType = selectedBoard.list.find(data => data.id === listId)!;
...
mock.onPost('/api/scrumboard/delete/board').reply((request: AxiosRequestConfig) => {
  const { boardId } = JSON.parse(request.data);
  boardData = boardData.filter(data => data.id !== boardId);
  return [200, boardId];
});

mock.onPost('/api/scrumboard/delete/list').reply((request: AxiosRequestConfig) => {

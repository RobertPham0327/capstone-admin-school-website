...
mock.onGet('/api/mailApp/mail/').reply((config: AxiosRequestConfig) => {
  const { params } = config;
  const response = mailList.find((mail: MailObjType) => mail.id === parseInt(params.id));
  return [200, response];
});
...
mock.onGet('/api/mailApp/folder/mail/List').reply((config: AxiosRequestConfig) => {
  const { params } = config;
  ...
  if (params.type === 'folder') {
    ...
    folderMailList = mailList.filter((mail: MailObjType) => mail.label.id === labelType);
  }
...
mock.onPut('/api/mailApp/update/starred').reply((request: AxiosRequestConfig) => {
  ...
  const updatedMails = mailList.filter((mail: MailObjType) => mailIds.includes(mail.id));
  return [200, updatedMails];
});

mock.onPut('/api/mailApp/update/folder').reply((request: AxiosRequestConfig) => {

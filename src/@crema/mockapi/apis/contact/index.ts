...
mock.onGet('/api/contactApp/contact/List').reply((config: AxiosRequestConfig) => {
  const { params } = config;
...
mock.onPut('/api/contactApp/update/starred').reply((request: AxiosRequestConfig) => {
...
mock.onPost('/api/contactApp/delete/contact').reply((request: AxiosRequestConfig) => {
  ...
  if (type === 'folder') {
    ...
    const labelType = labelList.find((label: LabelObjType) => label.alias === name)?.id;
    contactList = contactList.filter(contact => !contactIds.includes(contact.id));
    folderContactList = contactList.filter(contact => contact.label === labelType);
  }
  const index = page * 15;
...
mock.onPut('/api/contactApp/update/label').reply((request: AxiosRequestConfig) => {

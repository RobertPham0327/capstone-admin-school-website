...
mock.onGet('/api/chatApp/connection/messages').reply((request: AxiosRequestConfig) => {
  const { id } = request.params;
  const response = chatData.find((chat: MessageObjType) => chat.channelId === parseInt(id));
  if (response) {
    return [200, response];
  }
  return [200, null];
...
mock.onPost('/api/chatApp/message').reply((request: AxiosRequestConfig) => {
  ...
  connectionData = connectionData.map((item: ConnectionObjType) => (item.channelId === user?.channelId ? user : item));
...
mock.onPut('/api/chatApp/message').reply((request: AxiosRequestConfig) => {
  const { channelId, message } = JSON.parse(request.data);
  let user = connectionData.find((connection: ConnectionObjType) => connection.channelId === channelId)!;
  ...
  if (userMessages)
    userMessages.messageData = userMessages.messageData.map((item: MessageDataObjType) =>
      item.id === message.id ? message : item,
    );
  return [200, { user, userMessages }];
});

mock.onPost('/api/chatApp/delete/message').reply((request: AxiosRequestConfig) => {
  ...
  if (userMessages) {
    ...
    if (user?.lastMessage?.id === messageId) {
      const lastMessage = userMessages.messageData[userMessages.messageData.length - 1] as MessageDataObjType;
      user = {
        ...user,
        lastMessage: {
          id: lastMessage.id!,
      ...
      connectionData = connectionData.map((item: ConnectionObjType) => (item.id === user?.id ? user : item))!;
    }
  }
  return [200, { user, userMessages }];
});

mock.onPost('/api/chatApp/delete/user/messages').reply((request: AxiosRequestConfig) => {

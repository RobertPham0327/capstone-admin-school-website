...
const CheckedMailActions: React.FC<CheckedMailActionsProps> = ({ checkedMails, setCheckedMails, setData }) => {
  ...
  const onSelectLabel = (key: number) => {
    const labelType = labelList.find((label: LabelObjType) => label.id.toString() === key.toString());
  ...
  const menuMoveTo = folderList?.map((folder: FolderObjType, index: number) => {
    return {
      key: index,
      label: <span onClick={() => onChangeMailFolder(folder.id)}>{folder.name}</span>,
    };
  });

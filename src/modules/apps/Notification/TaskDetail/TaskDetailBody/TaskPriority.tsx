...
const TaskPriority: React.FC<TaskPriorityProps> = ({ selectedTask }) => {
  ...
  const priorityList = useAppSelector(({ calendarApp }) => calendarApp.priorityList);

  const onChangePriority = (value: number) => {
    const priority = priorityList.find((priority: PriorityObjType) => priority.type === value) as PriorityObjType;
